...
def main():
  ...
  parser.add_option('-g', dest='gaps_bed',
      default=None,
      help='Genome assembly gaps BED; coverage in those intervals clipped like blacklist.')
  ...
  gaps_chr_trees = read_blacklist(options.gaps_bed, black_buffer=0) if options.gaps_bed else {}
  ...
  for si in range(num_seqs):
    ...
    # set assembly gaps to baseline (same clipping as blacklist)
    if mseq.chr in gaps_chr_trees:
      for gap_interval in gaps_chr_trees[mseq.chr][mseq.start:mseq.end]:
        gap_seq_start = gap_interval.begin - mseq.start
        gap_seq_end = gap_interval.end - mseq.start
        gap_seq_values = seq_cov_nt[gap_seq_start:gap_seq_end]
        seq_cov_nt[gap_seq_start:gap_seq_end] = np.clip(
            gap_seq_values, -baseline_cov, baseline_cov)

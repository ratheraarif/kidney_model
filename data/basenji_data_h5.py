...
_BASENJI_DATA_READ_PY = os.path.join(_THIS_DIR, 'basenji_data_read.py')
_BASENJI_DATA_WRITE_H5_PY = os.path.join(_THIS_DIR, 'basenji_data_write_h5.py')
...
def main():
  ...
  for ti in range(targets_df.shape[0]):
    ...
    if options.restart and os.path.isfile(seqs_cov_file):
      ...
      cmd = '%s %s' % (shlex.quote(sys.executable), shlex.quote(_BASENJI_DATA_READ_PY))
  ...
  # initialize HDF5 output dir
  h5_dir = '%s/h5py' % options.out_dir
  ...
  for fold_set in fold_labels:
    ...
    h5_i = 0
    h5_start = fold_set_start
    h5_end = min(h5_start+options.seqs_per_tfr, fold_set_end)

    while h5_start <= fold_set_end:
      h5_stem = '%s/%s-%d' % (h5_dir, fold_set, h5_i)

      h5_file = '%s.h5' % h5_stem
      if options.restart and os.path.isfile(h5_file):
        print('Skipping existing %s' % h5_file, file=sys.stderr)
      else:
        cmd = '%s %s' % (shlex.quote(sys.executable), shlex.quote(_BASENJI_DATA_WRITE_H5_PY))
        cmd += ' -s %d' % h5_start
        cmd += ' -e %d' % h5_end
        ...
        if options.run_local:
          ...
          j = slurm.Job(cmd,
                name='write_%s-%d' % (fold_set, h5_i),
                out_file='%s.out' % h5_stem,
                err_file='%s.err' % h5_stem,
                queue='standard', mem=15000, time='12:0:0')
          write_jobs.append(j)

      # update
      h5_i += 1
      h5_start += options.seqs_per_tfr
      h5_end = min(h5_start+options.seqs_per_tfr, fold_set_end)
